...
function buildCharts(sample) {
  ...
  d3.json("samples.json").then((data) => {
    ...
    // @TODO Do I need to parse to a float?  Looks like it's a float in samples.json
    ...
    var yticks = resultOTUIDs.slice(0,10).reverse().map(ID => ("OTU " + ID));

    // Get the top 10 result sample values, sorted ascending
    var sortedSampleValues = resultSampleValues.slice(0,10).reverse();

    // Get the top 10 result OTU labels, sorted ascending
    var sortedOTULabels = resultOTULabels.slice(0,10).reverse();
    ...
    var barLayout = {
      title: '<b>Top 10 Bacteria Cultures Found</b> <br> If fewer than 10 were found, all are shown'     
    };
    ...
    // Gauge Chart 4. Create the trace for the gauge chart.
    var gaugeData = [{
      domain: {x: [0, 1], y: [0, 1] },
      value: wfreq,
